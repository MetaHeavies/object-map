...
- Semantic colors are amber for attributes, blue for relationships, green for actions and lilac for optional states. Every colour is OKLCH: one lightness ramp at four hues, so a row's label is a darker tone of the row itself rather than a neutral grey laid over it. Reduced-motion and keyboard selection respond immediately.
