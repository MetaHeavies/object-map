...
5. **Extract relationships.** Trace both direct references and join-backed associations. Capture the role at the source object (Owner, Shared with) and the actual target object (Person). Verify direction; many-to-many tables often reveal relationships absent from direct foreign keys. Avoid duplicating a foreign-key attribute when the relationship already represents it.
   ...
   Name the role for what the relationship means at the source: Taken at, Owned by, Located in, Shared with, Part of, About. The role, not the target's name, is what the builder reads. Reuse the target's name only when the relationship genuinely carries no distinguishing meaning and is the sole link to that target; treat a map where most roles repeat their target as a sign that meaning was not extracted.
