...
Optional `destination` and `within` on an object record where a person meets it. `destination: true` means the object has a place of its own a person can go to — a route, a list, a page. `within` is an array of object IDs whose views this object appears inside, so an Invoice a person only ever meets on a project screen is `"within": ["obj:project"]`, and one that also appears in treasury is `["obj:project", "obj:treasury"]`. Both are read from routes and views and are checkable the same way `filterable` is: name the route or the component, or leave the field off. Missing means unassessed, not absent. An object cannot be within itself, and every entry must be an existing object.
